...
pub fn get_gatherers(settings: &HashMap<GathererKey, AgentSettings>) -> Vec<Box<(dyn common::PullseGatherer + Send)>> {
    let mut result = Vec::new();

    if let Some(weather_settings) = settings.get("weather") {
        if weather_settings.enabled {
            result.push(Box::new(
                weather::WeatherDataGatherer::new(&weather_settings.options)
